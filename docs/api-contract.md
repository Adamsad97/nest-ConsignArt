...
Legend: 🌐 public (no token) · 🔒 any authenticated user · role names = restricted to that role (or `admin`, always allowed).
...
| POST | `/auth/login` | 🌐 | 200 | 401 invalid credentials / inactive account |
| POST | `/auth/refresh` | 🌐 | 200 | 401 invalid or expired refresh token |
...
| GET | `/users/:id` | 🔒 (self, or admin for any) | 200 | 401, 404 |
| PATCH | `/users/:id` | 🔒 (self, or admin for any) | 200 | 400 (unknown field rejected — `role`/`password` are not updatable here) |
...
| Method | Path | Access | Success | Errors |
|---|---|---|---|---|
| GET | `/artworks` | 🌐 (cached 30s) | 200 | — |
| GET | `/artworks/:id` | 🌐 (cached 30s) | 200 | 404 |
